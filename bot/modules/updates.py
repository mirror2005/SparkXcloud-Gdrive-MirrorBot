...
async def update_it(client, message):
    msg_ = await message.reply_text("`Succuesfully connected to Jetbots.org, Updating Please Wait!`")
    text = message.text.split(None, 1)[1]
    try:
        repo = Repo()
    except GitCommandError:
        return await msg_.edit(
            "**Invalid Git Command. Please Report This Bug To [Channel](https://t.me/jetbots)**"
        )
